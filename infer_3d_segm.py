...
注意：
- 推理时会把体数据 resize 到 --model-input-size，再把预测 mask resize 回原始尺寸。
...
def resize_image_trilinear(image: torch.Tensor, size: Tuple[int, int, int]) -> torch.Tensor:
	"""(1,D,H,W) -> (1,d,h,w)，使用 trilinear。"""
	x = image.unsqueeze(0)  # (1,1,D,H,W)
	x = F.interpolate(x, size=size, mode="trilinear", align_corners=False)
	return x.squeeze(0)


def resize_mask_nearest(mask: torch.Tensor, size: Tuple[int, int, int]) -> torch.Tensor:
	"""(D,H,W) 或 (1,D,H,W) -> (D,H,W)，使用 nearest。"""
	if mask.dim() == 3:
		x = mask.unsqueeze(0).unsqueeze(0)
	elif mask.dim() == 4:
		x = mask.unsqueeze(0)
	else:
		raise ValueError(f"Unexpected mask dim: {mask.shape}")
	x = F.interpolate(x.float(), size=size, mode="nearest")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
	...
	model_input_size = tuple(int(x) for x in args.model_input_size)
	...
	if ckpt_path is None:
		best, score = get_best_checkpoint([args.checkpoint_dir], name_prefix=net_name)
		if best is None:
			raise FileNotFoundError(
				f"未指定 --checkpoint，且在 {args.checkpoint_dir} 没找到 {net_name}*.ckpt"
			)
		print(f"自动选择 best ckpt：{best} (val_dice={score})")
		ckpt_path = best
	...
	for p in test_paths:
		...
		# (D,H,W) -> (1,D,H,W)
		x = torch.from_numpy(vol).half().div_(float(args.intensity_divisor)).unsqueeze(0)
		x = x.to(device)

		# resize 到模型输入尺寸
		x_rs = resize_image_trilinear(x, model_input_size)
		x_rs = x_rs.unsqueeze(0)  # (1,1,d,h,w)
		...
		pred_restore = resize_mask_nearest(pred_bin, original_shape).to(torch.uint8)
		tifffile.imwrite(str(masks_dir / f"{case_id}.tif"), pred_restore.cpu().numpy())
